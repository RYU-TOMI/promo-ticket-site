...
| **CH2** | **필터·정렬** | `when` 비교(B26) · 날짜 8칩 · 며칠 축 · 예산 범위·바로가기 칩 · 분위기 6종 · 0곳 칩 비활성 · 접힌 도크 조건 표시 · 필터 시 far 뷰 · 태그 사진 위 · 도크 라벨 제외 | **거의 완료** — 히스토그램만 대기 |
...
| T10 | **예산 히스토그램** | ⏸ **기획 답 대기** — 눈금이 `SPEC.md:415`(로그 기각) ↔ `SPEC.md:1052`(로그 채택)로 어긋난다 |

- 로드맵의 "검색 한 줄 재설계"는 **뺐다.** `SPEC.md:328`이 "위치 = 지도 우측 하단 플로팅 도크(헤더 바 아님)"로
  확정하면서 대체됐다. 로드맵 쪽이 확정 전 메모였다.
- **아직 눈으로 못 본 것**: T6~T9는 브라우저 확장이 연결되지 않은 시간대에 작업해 **화면 확인을 못 했다.**
  수치 검증(픽스처 대조·정적 계산)만 마쳤다. 사용자 확인 필요 항목은 각 커밋 메시지의 "눈으로 볼 것"에 적었다.
