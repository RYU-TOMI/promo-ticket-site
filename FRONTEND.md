...
### 8-8. 🔴 M2 T5 「diff 0」은 성립하지 않는다 — 검수 전제를 미리 박아둔다

**M2 T5의 이름이 「diff 0 증명」이지만, 노선 페이지 36장 중 22장은 반드시 달라진다.**
이건 이전이 깨뜨리는 게 아니라 **계약이 의도한 변경**이다. 모르고 검수에 들어가면
22장을 전부 회귀로 의심하게 되고, 반대로 뭉뚱그리면 진짜 회귀를 놓친다.

#### 무엇이 달라지나 (실측, `data/prices.db`, 2026-09-08)

| 무엇을 바꾸나 | 변화 노선 | 늘어남 | 줄어듦 |
|---|---|---|---|
| `LIMIT 10`만 제거 | 15/36 | 15 | 0 |
| `HAVING COUNT(*)>=3`만 제거 | 5/36 | 5 | 0 |
| **둘 다 제거 (계약 M1 지시 1)** | **22/36** | 22 | 0 |
| **M1 + BB29 창(`depart_date >= 오늘`)** | **22/36** | 10 | **12** |

**내가 처음 낸 15/36은 불완전했다.** `LIMIT 10`만 빼고 재서 그랬고,
계약은 `HAVING`도 같이 뺀다. 백엔드 세션이 잡아줬고 네 숫자 전부 재현했다.

#### 🔴 방향이 뒤집힌다 — 「늘어난다」로 잡으면 12장을 오인한다

BB29(백엔드 발견)를 같이 적용하면 **절반 이상이 오히려 줄어든다.**
`month_min`에 `depart_date` 창이 없어서 **이미 지난 달 버킷이 살아 있기** 때문이다.

배포본에서 실제로 확인한 것(오늘 2026-09-08):
...
**5장이 지금 지난 달에 떠나라고 쓰고 있다.** 임계로는 못 막는다 —
`ICN-NRT`의 2026-08 버킷은 915건이라 `n>=3`을 여유롭게 통과한다.
**창으로 잘라야 하고, 계약상 창은 백엔드 소유다.**

줄어드는 쪽 예: `ICN-NRT 8→6` · `ICN-KIX 9→7` · `ICN-TPE 9→7` · `ICN-SGN 9→7` ·
`ICN-HAN 10→8` · `ICN-FUK 10→9` · `ICN-DAD 4→3`

#### 검수할 때 지킬 것

1. **T5의 판정은 「diff 0」이 아니라 「diff가 예상 목록과 일치하는가」로 바꾼다.**
   위 22장 = 예상. **그 밖의 페이지에 한 줄이라도 diff가 나면 그게 회귀다.**
2. 22장 안에서도 **막대 개수 말고 다른 게 바뀌면 회귀다** — 가격·문장·표·차트 축.
3. **BB29가 M1에 같이 들어갔는지 먼저 확인한다.** 안 들어갔으면 페이지가 두 번 흔들린다
   (M1에서 늘고, 나중에 줄고). 그러면 예상 목록도 두 번 만들어야 한다.
4. 요일 차트는 `ICN-RMQ` **한 장만** 바뀐다(5→3). 백엔드가 전 노선 재현으로 확인했고
   나도 같은 값을 얻었다. **다른 노선의 요일 차트가 바뀌면 회귀다.**

#### 겸사겸사 — 내 임계는 창을 대신하지 못한다

`usable(buckets)=length>=2` · `enough(bucket)=n>=3`은 **얇은 것**만 막는다.
**낡은 것**은 못 막는다(915건짜리 지난 달 버킷). 둘은 다른 문제다.
M2에서 판정 함수를 만들 때 「받은 `months`는 이미 창으로 잘려 있다」를 전제로 삼는다 —
프론트가 날짜로 다시 거르면 창이 두 곳에 생기고, 두 곳에 있으면 갈라진다.
