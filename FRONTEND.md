...
#### 🔴 diff 0 이지만 무변경은 아니다 — 잠복 **변경** 둘 + 잠복 **부채** 하나
...
| 예 | BB28 · `dateModified` | **`inline_deals()`** |
...
| **`dateModified`·`lastmod`** `today_utc()` → `generated[:10]` | 오늘 둘 다 같은 날짜 | 크론이 UTC 자정 근처로 옮겨갈 때. **그때 `generated` 쪽이 옳다** |
...
**만들 수 있었다. 실물을 보고 추측한 필드 0개.** 계약에 없었으면 조용히 틀렸을 자리 셋:
