...
- `main`에는 **크론(github-actions)이 매일 `data/`·`docs/`를 커밋**한다 → push 전 반드시 pull.
  ...
  - 재빌드했으면 **커밋 전에 확인한다**: `grep -o '"ad":true' docs/data/deals.json | wc -l` → **125**
    > ⚠️ `grep -c`를 쓰지 말 것. `deals.json`은 **개행이 없는 한 줄**이라 `grep -c`는 매칭된
    > *줄 수*를 세서 **건수와 무관하게 항상 `1`**이다. 전량 손실만 걸리고 **부분 손실은 통과한다**
    > (125건 중 3건만 남아도 `1`). 2026-09-08 이 문서가 실제로 `grep -c`로 잘못 적혀 있었고
    > 백엔드·프론트가 각각 잡았다.
