# -*- coding: utf-8 -*-
"""프로모션 메일 본문을 Claude로 파싱해 구조화된 특가 정보를 추출.
...
class MailParseResult(BaseModel):
    ...
    official_url: str | None = Field(
        ...
        description="항공사 공식 프로모션/이벤트 페이지 URL. 반드시 항공사 도메인의 일반 URL만. "
                    "추적 파라미터·수신자 식별자가 포함된 링크나 구독해지 링크는 절대 넣지 말 것")
...
def parse_one(client, sender, subject, body_html):
    prompt = (
        ...
        "- 단순 회원가입 안내, 보안 알림, 약관 변경 등 특가와 무관하면 is_deal=false\n"
        "- 가격은 원화 숫자만 (예: '129,000원~' -> 129000)\n"
