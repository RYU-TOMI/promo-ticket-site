...
- [ ] **파트2. 홈=풀스크린 지도**: `theme.py` 셸 + 헤더/네비 + 화면0(출발지 선택). 개발 중 JSON 인라인.
- [ ] **파트3. 핀 데이터·인코딩**: broad_offers → deals.json(데이터 게이팅) + 핀(**크기 고정·색=가격**, tier major/minor).
- [ ] **파트4. 단계 줌 + LOD**: 가까운곳→동남아→유럽·미주 projection **트윈**, 배율 임계값으로 minor 자동 등장.
- [ ] **파트5. 호버(탭) → 항로+카드**: 곡선 draw + 사진(lazy webp)·최저가(~라벨)·태그·예약 링크 + 상세.
- [ ] **파트6. 마감**: 특가 피드·구독 통합, 모바일 바텀시트, noscript 대체, 50m 해안선, 배포.
...
- [ ] **화면0 출발지 선택 UX**: 한국 지도 5핀 vs 드롭다운, 모바일 방식. (지도 목업은 서울 고정 상태)
- [ ] **필터 동작**: 분위기(해변/도시/온천)·예산대 칩이 핀을 **숨김**인지 **흐림**인지, 예산 슬라이더 여부.
- [ ] **날짜 입력 여부**: 순수 발견(날짜 없음)인지, 상세에서만 날짜 선택인지.
- [ ] **밀집 라벨 규칙**: minor 라벨 상시 vs 호버 시에만(스토리보드 ⑤ 푸켓·크라비 겹침 참고).
- [ ] **LOD 임계값**: 현재 목업 scale≈1200 — 실지도에서 미세조정.
- [ ] **IA**: 네비(발견/오늘의특가/노선별/알림)가 별도 페이지인지 홈 스크롤 섹션인지.
- [ ] **빈 상태**: 필터/지역에 특가 0건일 때 문구·대체.
