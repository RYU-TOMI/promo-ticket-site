...
- [ ] **파트5. 연동 + 카드**: 피드↔핀 양방향 하이라이트 + 항로 곡선 draw + 플로팅 카드(사진 lazy webp·최저가~·날짜·예약) + 상세.
- [ ] **파트6. 마감**: 구독 통합, 모바일, noscript 대체, 50m 해안선, 배포.
...
- [ ] **화면0 출발지 선택 UX**: 한국 지도 5핀 vs 드롭다운, 모바일 방식. (목업은 서울 고정 상태)
- [ ] **필터 동작**: 분위기(해변/도시/온천)·예산대 칩이 핀/카드를 **숨김**인지 **흐림**인지, 예산 슬라이더 여부.
- [ ] **피드 정렬**: 기본 가성비순 확정. 임박순·할인율순 토글 제공 여부.
- [ ] **갈래/말래 범위**: hero만 vs 전 카드. "말래"의 동작(그냥 넘김 vs 학습해서 추천 개인화).
