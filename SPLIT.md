...
**기준선 SHA는 M2 착수 시 프론트가 정해 여기 적는다.** 정해지기 전에는 검수를 시작하지 않는다.
...
**M1과 CH6는 병렬 가능하다** (백엔드 확인 2026-09-08):
- 파일 교집합이 없다 — M1은 `build_site.py`·`affiliates.py`·`tests/`, CH6는 프론트 구역
- 의존 방향이 한쪽이다 — **M2가 M1을 기다리지, M1이 프론트를 기다리지 않는다**
- 순서는 사용자 편의대로. 단 **CH6가 배포되면 M2 기준선(§M2 T5)을 다시 잡는다**
- ⚠️ 둘 다 `main`에 병합하므로 `docs/` 충돌이 날 수 있다 → `CLAUDE.md`의 새 규칙
  (시크릿 없으면 재빌드 금지, 백엔드에 요청)이 그대로 적용된다. **그 규칙의 첫 시험대다**
